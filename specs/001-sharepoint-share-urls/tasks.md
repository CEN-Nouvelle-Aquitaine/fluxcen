...
- [X] T038 Découverte automatique de la configuration Microsoft dans le gestionnaire d'auth QGIS (issue #39) : `select_web_authcfg()` dans `core/ms_urls.py` (tests purs), `_authcfg_id()` interroge le gestionnaire au lieu d'exiger `auth.authcfg` dans links.yaml (la clé reste une surcharge optionnelle, tests d'intégration dans `tests/test_fetch.py`) ; message `AUTH_MANQUANTE` ramené à la seule action à portée de l'utilisateur (data-model) ; contrats et `links_example.yaml` mis à jour. **Remplacée par T039** : la découverte par méthode confondait toute config OAuth2 avec la config CEN (contradiction par agents adversariaux)
- [X] T039 [FR-013] Provisionnement automatique de la configuration Microsoft canonique : `core/entra.py` (config PKCE sans secret, ID fixe `g2b2197`, ports 17070+ avec sonde d'occupation — AnyDesk écoute sur 7070), `_ensure_microsoft_authcfg()` crée/répare la config au premier accès Microsoft (jamais au démarrage), surcharge links.yaml validée contre le gestionnaire, familles d'erreurs `AUTH_PROVISIONNEMENT` et `PORT_REDIRECTION` ; `select_web_authcfg()` supprimée ; spec (FR-013), data-model, contrats et `links_example.yaml` amendés ; validation en réel du flux PKCE sans secret le 2026-08-17
- [X] T040 [FR-011] Cache de session pour l'auth BDD/services sécurisés (revue de PR) : suppression du défaut persistant QSettings (`FluxCEN/default_auth_id`), de `choose_default_authentication()`, du bouton 🛠️ (`commandLinkButton_5`, UI comprise) et de `check_authentication_configs()` au démarrage ; `_select_service_authcfg()` mémorise le choix utilisateur pour la session (`_selected_service_authcfg`) avec re-résolution vivante si la config disparaît ; tests réécrits dans `tests/test_postgis_auth.py` + gardes de non-retour ; `apply_authentication_if_needed()` supprimée (`_select_service_authcfg()` appelée directement dans `handle_postgis_layer`, comme pour WMS/WFS) ; FR-011 et research.md amendés
- [X] T041 [FR-012] Couche sécurisée sans config adaptée : jamais créée ni ajoutée au projet (suggestions de revue appliquées dans `handle_wms_layer`/`handle_wfs_layer`) — log critique affiché au lieu d'une couche vide + fenêtre d'identification native ; tests d'intégration `tests/test_secured_layers.py` ; clarification ajoutée à FR-012
